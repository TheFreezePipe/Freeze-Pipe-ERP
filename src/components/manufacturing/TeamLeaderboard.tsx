...
import type { EmployeeSummary, TaskType } from "@/lib/performance/aggregate";
...
function EmployeeDetailDialog({ summary, onClose, rangeLabel }: DialogProps) {
  const PAGE_SIZE = 50;
  const [page, setPage] = useState(0);

  // Reset pagination whenever a new employee is opened.
  const key = summary?.employeeId ?? "";
  useResetOnChange(key, () => setPage(0));
  ...
  return (
    <Dialog open={!!summary} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{summary.employeeName}</DialogTitle>
              ...
              {visible.map(t => {
                ...
                return (
                  <tr key={t.id} className="border-b border-border/40">
                    ...
                    <td className="py-2 text-muted-foreground text-xs">{t.sku_name ?? t.sku_id}</td>
...
function useResetOnChange(key: string, reset: () => void) {
  // Inline hook to avoid an effect import dance in the already-long file.
  // Using a ref would be cleaner, but simpler: just compare via state.
